...
[@ReminderPolicy specification](ReminderPolicy.md)

[@ReminderPolicy implementation](/src/concepts/ReminderPolicy/ReminderPolicyConcept.ts)

Please create comprehensive tests for the ReminderPolicy concept.

Required test scenarios:

1. Operational principle test: Create policy → select policy by tags → instantiate reminders → learn from behavior → verify adaptation
   - Should demonstrate the full learning loop described in the spec

2. Policy creation and selection: Test creating policies with different tag combinations
   - Test selection with exact matches, partial matches, no matches
   - Test default policy fallback

3. Reminder instantiation: Test generating reminders from policies
   - Without personalization (initial state)
   - With personalization (after learning)
   - Verify blending formula is applied correctly

4. Learning algorithm: Test the learn action with various feedback
   - Positive feedback (wasEffective = true) increases effectiveness
   - Negative feedback (wasEffective = false) decreases effectiveness
   - Verify exponential moving average updates

5. Policy management: Test update and delete operations
   - Update policy rules and verify future instantiations use new rules
   - Delete policy and verify default policy mappings are cleaned up

6. Edge cases: Multiple policies for same tags, effectiveness bounds (0.0-1.0), empty rule sets
