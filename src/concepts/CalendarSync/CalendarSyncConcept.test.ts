import { assert, assertEquals, assertExists, assertNotEquals, assertObjectMatch } from "jsr:@std/assert";
...
Deno.test("CalendarSyncConcept: Operational Principle Trace", async (test) => {
  ...
  await test.step("4. Suggest reschedules and select 3pm Tuesday", async () => {
    ...
    selectedReschedule = suggestionList[0];
    assertExists(selectedReschedule, "At least one reschedule suggestion should be available.");
  });

  await test.step("5. Commit event at selected reschedule to both Google and Outlook", async () => {
...
Deno.test("CalendarSyncConcept: Event Commitment", async (test) => {
  ...
  await test.step("5. Commit event to a non-existent account - expect error", async () => {
    ...
    assertExists(result);
    assertNotEquals((result as { error: string }).error, undefined);
    assert((result as { error: string }).error.includes("non-existent-id") || (result as { error: string }).error.includes("not connected"));
  });
...
Deno.test("CalendarSyncConcept: Conflict Detection", async (test) => {
  ...
  await test.step("6. Detect conflicts for a new event overlapping partially with the internally committed event", async () => {
    const draft = createTestDraftEvent(18, 19, "Partial Evening Event", 0); // July 9th, 6pm-7pm (full overlap)
    const conflicts = await concept.detectConflicts({ user: USER_ALICE, draft });
    assertExists(conflicts);
    assertEquals(Array.isArray(conflicts), true);
    assert((conflicts as Conflict[]).length >= 1, "Should have at least one conflict");
    const hasMatchingConflict = (conflicts as Conflict[]).some(c => 
      c.existingEvents.includes(committedEventId) || c.reason.includes("internal event")
    );
    assert(hasMatchingConflict, "Should detect conflict with committed event");
  });
...
Deno.test("CalendarSyncConcept: Reschedule Suggestions", async (test) => {
  ...
  await test.step("3. Suggest reschedules for a draft event that conflicts with Carol's Morning Meeting (10:30am Tuesday)", async () => {
    const conflictingDraft = createTestDraftEvent(10, 11, "Carol's Draft Conflict", 0); // 10am-11am
    const suggestions = await concept.suggestReschedules({
      user: USER_CAROL,
    ...
    assertExists(suggestions);
    assertEquals((suggestions as { error?: string }).error, undefined);
    assert((suggestions as Timespan[]).length > 0, "Should have at least one suggestion");

    const suggestionList = suggestions as Timespan[];

    // Verify suggestions avoid the 10am-11am conflict
    for (const suggestion of suggestionList) {
      const testDraft = { ...conflictingDraft, start: suggestion.start, end: suggestion.end };
      const conflicts = await concept.detectConflicts({ user: USER_CAROL, draft: testDraft });
      assertEquals((conflicts as Conflict[]).length, 0, `Suggestion at ${suggestion.start.getHours()}:${suggestion.start.getMinutes()} should have no conflicts`);
    }
  });
