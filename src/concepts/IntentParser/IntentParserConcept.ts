...
export default class IntentParserConcept {
  ...
  async parseManually(
    ...
    const inMatch = utterance.match(/(?:in|at)\s+([\w\d\s.-]+(?:room|building|zoom|meet|office|cafe|restaurant|library|[A-Z]))/i);
  ...
  async refineWithAI(
    ...
    try {
      ...
      // MongoDB deserializes Sets as arrays or objects, so convert them back
      const participantsArray = Array.isArray(existingParsed.draftEvent.participants) 
        ? existingParsed.draftEvent.participants 
        : Object.values(existingParsed.draftEvent.participants || {});
      const tagsArray = Array.isArray(existingParsed.draftEvent.tags)
        ? existingParsed.draftEvent.tags
        : Object.values(existingParsed.draftEvent.tags || {});
      
      const existingDraftWithSets: DraftEvent = {
        ...existingParsed.draftEvent,
      ...
      const { draftEvent, alternatives, confidence } = this._mockLLMParse(
        ...
        existingDraftWithSets, // Pass existing draft for refinement context
      );
      // --- END MOCK INTEGRATION ---
