...
content_id: 83aac7ed12ccd00dbe74dfe61754b943b21f1ad3b3f51b4c54eb6a9497c36f50
---

# file: src/concepts/EventDrafts/EventDraftsConcept.ts
...
Please create comprehensive tests for the EventDrafts concept following the testing guidelines.

Required test scenarios:

1. Operational principle test: Create draft → validate → mark validated → retrieve → delete
   * Should cover the common workflow described in the spec

2. Create and retrieve: Test creating drafts with various inputs and retrieving them
   * Include edge cases like empty attendees, different time ranges

3. Validation logic: Test the validate action with valid and invalid drafts
   * Valid: proper time ordering, non-empty title
   * Invalid: end before start, empty title

4. Constraints management: Test attaching and updating constraints
   * Zero buffers, non-zero buffers, room requirements, virtual links

5. Update and re-validation: Test updating drafts and status reset behavior
   * Verify status returns to "Created" after updates

6. Error cases: Test actions with invalid inputs
   * Non-existent draft IDs, invalid status transitions

Use Deno testing framework with:

* Programmatic assertions using `assertEquals`, `assertExists`, etc.
* Console output showing action inputs and results for human readability
* Test database cleanup between scenarios using testDb()
